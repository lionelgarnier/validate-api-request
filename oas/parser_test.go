...
func TestParseJSON(t *testing.T) {
	...
	assert.Equal(t, "Sample API", openAPI.Info.Title)
...
func TestParseComplexJSON(t *testing.T) {
	...
	assert.Equal(t, "Swagger Petstore - OpenAPI 3.0", openAPI.Info.Title)
	assert.Equal(t, "1.0.11", openAPI.Info.Version)
	assert.Equal(t, "Find out more about Swagger", openAPI.ExternalDocs.Description)
	assert.Equal(t, "https://petstore3.swagger.io/api/v3", openAPI.Servers[0].URL)
	assert.Equal(t, "Returns pet inventories by status", openAPI.Paths["/store/inventory"].Get.Summary)
	assert.Equal(t, "successful operation", openAPI.Paths["/store/inventory"].Get.Responses["200"].Description)
	assert.Equal(t, "integer", openAPI.Components.Schemas["Order"].Properties["id"].Type)
